 ...
 */
async function loadCatalogFile(filePath) {
 ...
 */
function mergeCatalogs(base, overlay) {
  if (!overlay) {
    return base;
  }
 ...
 */
async function loadDiscoveredCatalog() {
 ...
 */
export async function getModel(modelIdOrAlias) {
  const catalog = await loadCatalog();

  // Try direct ID lookup
  if (catalog.models[modelIdOrAlias]) {
    return { ...catalog.models[modelIdOrAlias], id: modelIdOrAlias };
  }
...
export async function listModels(filter) {
  const catalog = await loadCatalog();
  let models = Object.entries(catalog.models).map(([id, model]) => ({ ...model, id }));
  ...
  if (filter.minContext) {
    models = models.filter((m) => m.capabilities.contextWindow >= filter.minContext);
  }
...
export async function searchModels(query) {
  ...
  return Object.entries(catalog.models).map(([id, model]) => ({ ...model, id })).filter((model) => {
    // Search in ID
